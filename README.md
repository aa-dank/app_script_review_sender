...
3. **Template Processing**  
   - **Template Application**: When a row has a `distribution_template_label`, the system first applies template values to empty cells in the spreadsheet itself. This ensures you can see what values were applied and that the history sheet contains complete data.
   - The **email body** is generated by fetching the specified HTML template from Google Drive, merging it with values obtained from the JSON `email_template_values` column as well as any additional columns.
   ...
   - If a **template_label** is specified, the system will first check for a matching template in the `distribution_templates` sheet and apply any default values to empty cells in the spreadsheet before processing the email.
...
6. **History Tracking**  
   Once an email is successfully sent, the corresponding row (with all applied template values) is moved to a history sheet (`sent_history`) along with a timestamp. This ensures the history contains the complete data used for the email distribution.
...
2. **Using Templates:**  
   ...
   - When processing emails, the system will automatically fill in any empty fields in the spreadsheet with values from the template
   - You can see the applied template values directly in your spreadsheet before and after processing
...
3. **Template Application Behavior:**  
   - **During Email Sending**: Template values are automatically applied to empty cells before sending emails. This ensures both the spreadsheet and history sheet contain complete data.
   - **Manual Application**: You can use the "Apply Templates to Pending Rows" option from the menu to apply templates without sending emails
   - **Visibility**: After template application, you can see exactly what values were filled in by reviewing the updated cells in your spreadsheet
...
1. **Send Pending Emails**  
   Processes all rows in the `distributions_to_send` sheet, applies templates to empty cells, sends emails, and moves processed rows (with complete data) to the history sheet.

2. **Apply Templates to Pending Rows**  
   Applies template values to empty fields in rows without sending any emails. This is useful for preparing a batch of distributions before sending and for previewing what template values will be applied.
...
- **Template Values JSON:** Always ensure that the JSON in the `email_template_values` column is valid. Use proper quotes and avoid extra escape characters. The system will attempt to sanitize malformed JSON but providing correctly formatted JSON is recommended.
...
- **Viewing Applied Templates:** After processing emails or applying templates manually, you can see exactly what template values were used by looking at the updated cells in your spreadsheet. The history sheet will also contain the complete data with all applied template values.
...
3. **Create Email Distributions:**  
   ...
   - Update your email templates on Google Drive with placeholders matching the keys from `email_template_values` or any additional column.
...
5. **Run the Distribution:**  
   ...
   - Template values will be automatically applied to empty cells before sending, allowing you to see the complete data used for each email.
...
- **JSON Parsing Errors:** Check that the JSON in `email_template_values` is correctly formatted. The system attempts to handle common issues like missing braces, but proper JSON is recommended.
...
- **Template Values Not Visible:** If you don't see template values applied to your spreadsheet, ensure:
  - The template label exactly matches a template in the `distribution_templates` sheet
  - The cells you expect to be filled are actually empty (not containing spaces or other invisible characters)
  - You have the proper permissions to edit the spreadsheet

---

## Appendix 1: Using Template Values in Email Templates

The `email_template_values` column lets you provide key/value pairs in JSON format to dynamically fill placeholders in your email body and subject templates. Think of it like setting variables in a simple arithmetic problem:

- In arithmetic, you might say "let x = 2" and then compute "x + 3" to get 5.
- Comparably, in your email templates you define variables via JSON. For instance, in your spreadsheet's "email_template_values" column you can supply:
